...
export default function ServicesPage() {
  return (
    <div className="page-shell bg-[#111] text-white">
      <SiteHeader />
      <main>
          ...
          {services.map((service, index) => {
            ...
            return (
              <div key={service.slug}>
                  <div className="grid min-h-[520px] grid-rows-[260px_1fr] gap-0 overflow-hidden bg-[#1c1c1c] md:min-h-0 md:h-[320px] md:grid-cols-[0.82fr_1fr] md:grid-rows-1">
                  ...
                  <div className={`flex h-full flex-col justify-center overflow-hidden p-7 md:p-10 ${reversed ? 'md:order-1' : ''}`}>
                    <p className="text-xs font-black uppercase tracking-[0.12em] text-[#9a6500]">{String(index + 1).padStart(2, '0')}</p>
                      <h2 className="display mt-3 text-3xl text-white sm:text-4xl">{service.title}</h2>
                      <p className="mt-4 line-clamp-2 max-w-md leading-6 text-[#c6c6c1]">{service.tagline}</p>
